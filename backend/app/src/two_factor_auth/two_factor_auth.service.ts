...
export class TwoFactorAuthService {
	...
	async get_secret(userId: number, session: any): Promise<[boolean, string]> {
		...
		let ret = await axios.get(`http://pgrest:${process.env.PGREST_PORT}/two_factor_auth?user_id=eq.${userId}`)
	...
	async get_keyuri(userId: number, session: any): Promise<string> {
		...
		let ret = await axios.get(`http://pgrest:${process.env.PGREST_PORT}/users?id=eq.${userId}`);
	...
	async login(userId: number, token: string, session: any): Promise<boolean> {
		...
		if (success && secret[0] == false) {
			...
			axios.post(`http://pgrest:${process.env.PGREST_PORT}/two_factor_auth`, {"user_id": userId, "seed": secret[1]})
				.then((response) => {
