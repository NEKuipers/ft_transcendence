...
export class ChannelsService {
	async findAll(): Promise<Channel[]> {
		let res = await axios.get(`http://${process.env.PGREST_HOST}:${process.env.PGREST_PORT}/channels?type=neq.direct&is_closed=eq.false`);
		return res.data;
	}
	...
	async findOne(channel_id: number) : Promise<Channel> {
		let res = await axios.get(`http://${process.env.PGREST_HOST}:${process.env.PGREST_PORT}/channels?id=eq.${channel_id}`)
		return res.data;
	}
	...
	async createChannel(channel: Channel): Promise<string> {
		...
		await axios.post(`http://${process.env.PGREST_HOST}:${process.env.PGREST_PORT}/channels`, {
			name: channel.name,
			type: channel.type,
	...
	async changeOwner(id: number, channel: Channel): Promise<string> {
		axios.patch(`http://${process.env.PGREST_HOST}:${process.env.PGREST_PORT}/channels?id=eq.${channel.id}`, {
			owner_id: id
		})
