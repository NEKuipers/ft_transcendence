...
export class BlockedUsersService {
	...
	findAllForUser(id: number) : Promise<BlockedUserVW[]> {
		return new Promise((accept, reject) => {
			axios.get(`http://pgrest:${process.env.PGREST_PORT}/vw_blocked_users?user_id=eq.${id}`)
				.then((response) => {
	...
	async haveYouBlockedUser(your_id: number, other_id: number) : Promise<boolean> {
		let res = await axios.get(`http://pgrest:${process.env.PGREST_PORT}/blocked_users?blocked_by_id=eq.${your_id}&blocked_user_id=eq.${other_id}`);
		return (res.data.length > 0);
	}

	async hasUserBlockedMe(your_id: number, other_id: number) : Promise<boolean> {
		let res = await axios.get(`http://pgrest:${process.env.PGREST_PORT}/blocked_users?blocked_user_id=eq.${your_id}&blocked_by_id=eq.${other_id}`);
		return (res.data.length > 0);
	}

	async getAllWhoBlockedMe(id: number) : Promise<number[]> {
		let res = await axios.get(`http://pgrest:${process.env.PGREST_PORT}/blocked_users?blocked_user_id=eq.${id}`);
		let ids = [];
	...
	async getAllWhoIHaveBlocked(id: number) : Promise<number[]> {
		let res = await axios.get(`http://pgrest:${process.env.PGREST_PORT}/blocked_users?blocked_by_id=eq.${id}`);
		let ids = [];
	...
	blockUser(blockedUser: BlockedUser) : string {
		axios.post(`http://pgrest:${process.env.PGREST_PORT}/blocked_users`, {
	...
	unblockUser(blockedUser: BlockedUser): string {
		axios.delete(`http://pgrest:${process.env.PGREST_PORT}/blocked_users?blocked_by_id=eq.${blockedUser.blocked_by_id}&blocked_user_id=eq.${blockedUser.blocked_user_id}`)
				.then(res => res)
				.catch(err => console.log(err));
		return "success";
	}
